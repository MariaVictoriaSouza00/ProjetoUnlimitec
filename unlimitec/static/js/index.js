document.addEventListener("DOMContentLoaded", () => {
  ...
  document.getElementById("btn-links").addEventListener("click", async () => {
    ...
    try {
      const response = await fetch(`/buscar_titulos_ajax/?termo=${encodeURIComponent(termoPesquisa)}`, {
          ...
          accordionItem.innerHTML = `
            ...
            <div id="collapse${index}" class="accordion-collapse collapse" data-bs-parent="#lista-chamadas">
              <div class="accordion-body">
                ...
                ${
                  item.link
                    ? `<a href="${item.link}" target="_blank" rel="noopener noreferrer">Acesse o edital</a>`
                    : ""
                }
              </div>
            </div>
  ...
  document.getElementById("btn-pesquisa").addEventListener("click", async () => {
    ...
    try {
      const resp = await fetch(`/definicao/?termo=${encodeURIComponent(termo)}`, {
        headers: { "X-Requested-With": "XMLHttpRequest" },
      });
